    ...
    <div class="container typecho-dashboard">
        ...
        <div class="row typecho-page-main">
            <div class="col-mb-12 welcome-board" role="main">
                <p><?php _e('Overview: <em>%s </em> article(s) with <em>%s </em> comments in  <em>%s </em> category(ies).',
                        $stat->myPublishedPostsNum, $stat->myPublishedCommentsNum, $stat->categoriesNum); ?>
                    <br><?php _e('Quick start (link below):'); ?></p>
                ...
                <ul id="start-link">
                    <?php if ($user->pass('contributor', true)): ?>
                        ...
                        <?php if ($user->pass('editor', true) && 'on' == $request->get('__typecho_all_comments') && $stat->waitingCommentsNum > 0): ?>
                            <li>
                                <a href="<?php $options->adminUrl('manage-comments.php?status=waiting'); ?>"><?php _e('Comments awaiting review.'); ?></a>
                        ...
                        <?php if ($user->pass('editor', true) && 'on' == $request->get('__typecho_all_comments') && $stat->spamCommentsNum > 0): ?>
                            <li>
                                <a href="<?php $options->adminUrl('manage-comments.php?status=spam'); ?>"><?php _e('Spam'); ?></a>
                        ...
                        <?php if ($user->pass('administrator', true)): ?>
                            ...
                            <li><a href="<?php $options->adminUrl('plugins.php'); ?>"><?php _e('manage plugins'); ?></a></li>
                            <li><a href="<?php $options->adminUrl('options-general.php'); ?>"><?php _e('System settings'); ?></a>
                            </li>
                        <?php endif; ?>
                    <?php endif; ?>
                ...
                <section class="latest-link">
                    ...
                    <ul>
                        <?php if ($posts->have()): ?>
                            ...
                            <?php endwhile; ?>
                        <?php else: ?>
                            <li><em><?php _e('No post yet.'); ?></em></li>
                        <?php endif; ?>
                    </ul>
                </section>
            </div>
            ...
            <div class="col-mb-12 col-tb-4" role="complementary">
                <section class="latest-link">
                    <h3><?php _e('Latest official blog posts.'); ?></h3>
        ...
        function applyUpdate(update) {
            if (update.available) {
                $('<div class="update-check message error"><p>'
                    + '<?php _e('You are using %s'); ?>'.replace('%s', update.current) + '<br />'
